...
std::shared_ptr<sortham::Optimizer> getDummyOptimizer(
  TNode node, TCostMap costmap_ros,
  TParamHandler * params_handler)
{
  std::shared_ptr<sortham::Optimizer> optimizer = std::make_shared<sortham::Optimizer>();
...
sortham::PathHandler getDummyPathHandler(
  TNode node, TCostMap costmap_ros, TFBuffer tf_buffer,
  TParamHandler * params_handler)
{
  sortham::PathHandler path_handler;
  std::weak_ptr<rclcpp_lifecycle::LifecycleNode> weak_ptr_node{node};
...
std::shared_ptr<nav2_sortham_controller::SORTHAMController> getDummyController(
  TNode node, TFBuffer tf_buffer,
  TCostMap costmap_ros)
{
  auto controller = std::make_shared<nav2_sortham_controller::SORTHAMController>();
  std::weak_ptr<rclcpp_lifecycle::LifecycleNode> weak_ptr_node{node};
